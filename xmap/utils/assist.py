...
def write_to_disk(results, out_dict, path):
    ...
    csv_path = join(path, "results.csv")
    ...
    resultName = "X-MAP-"
    isUser = out_dict['recommender']['calculate_xmap_sim_method_2']
    isXMAP = out_dict['recommender']['private_flag']
    ...
    if("user" in isUser):
        resultName = resultName + "UB"
    else:
        resultName = resultName + "IB"

    changeParam = out_dict['recommender']['test_param']

    valueParam = out_dict['recommender']['test_param_v']

    mae = out_dict['result']['mae']

    finalCSVLine = str(resultName) + "," + str(changeParam) + "," + str(valueParam) + "," + str(mae) + "\n"
    
    f = open(csv_path, "a")
    f.write(finalCSVLine)
    print("csv inserted")
    f.close()
